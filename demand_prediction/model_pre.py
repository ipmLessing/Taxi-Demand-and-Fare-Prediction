...
def model_predict(time_string):
    ...
    class MDN(layers.Layer):
        ...
        def __init__(self, output_dimension, num_mixtures, **kwargs):
            ...
            with tf.name_scope('MDN'):
                ...
                self.mdn_sigmas = layers.Dense(self.num_mix * self.output_dim, activation=elu_plus_one_plus_epsilon,
    ...
    def nparray2point(x):
        points = []
        for ll in x:
            geopoint = Point(ll[0], ll[1])
            points = points + [geopoint]
        return points

    model = load_model('Model_M=40_n1=100_n2=100sigmoid.h5',
                       custom_objects={"MDN": MDN, "mdn_loss_func": get_mixture_loss_func(OUTPUT_DIMS, N_MIXES)})

    date_time = pd.to_datetime(time_string)
    datate_time = pd.to_datetime(time_string)
    input = []
    for i in range(5000):  # Multiple sampling
        input.append([datate_time.month, datate_time.day, datate_time.hour, datate_time.minute, datate_time.second])
    output = model.predict(input)

    output_samples = np.apply_along_axis(sample_from_output, 1, output, OUTPUT_DIMS, N_MIXES, temp=1.0, sigma_temp=1.0)
    output_samples = output_samples.reshape(output_samples.shape[0], output_samples.shape[2])
    predict_points = nparray2point(output_samples)
    predict_pointsDF = geopandas.GeoDataFrame(geometry=predict_points)
    predict_y_zone = geopandas.sjoin(predict_pointsDF, data, how='left', op="within")['LocationID'].values

    output_dic = {j + 1: 0 for j in range(263)}
    for i in range(len(predict_y_zone)):
        if not np.isnan(predict_y_zone[i]):
            # print(predict_y_zone[i])
            output_dic[predict_y_zone[i]] = output_dic[predict_y_zone[i]] + 1
        else:
            pass
